...
pub struct SomeipMessageHeader {
    ...
    pub tp_header: Option<SomeipTpHeader>,
}

impl SomeipMessageHeader {
    ...
    pub fn read_base_from(buf: &mut BytesMut) -> SomeipMessageHeader {
    ...
    pub fn read_tp(&mut self, buf: &mut BytesMut) {
        ...
        let tpr = buf.get_u32();
        self.tp_header = Some( SomeipTpHeader { offset: (tpr >> 4), more: (tpr & 0x1) != 0 })
    }
}
...
/// Scale factor for [SomeipTpHeader::offset] value.
...
pub fn make_key(header: &SomeipMessageHeader) -> SegmentationKey {

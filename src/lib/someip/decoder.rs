...
pub struct SomeipDecoder {
    ...
    current_header: Option<SomeipMessageHeader>,
    wait_for_tp: bool,
}
...
impl Decoder for SomeipDecoder {
    type Message = (SomeipMessageHeader, Bytes);
    ...
    fn decode(&mut self, buf: &mut BytesMut) -> Result<Option<Self::Message>, Self::Error> {
        ...
        if self.current_header.is_none() {
            ...
            let header = SomeipMessageHeader::read_base_from(buf);
            if header.length < SOMEIP_HEADER_LEN_PART {
                return Err(SomeipWireError::LengthFieldErrorToShort)
            }
            if (header.length - SOMEIP_HEADER_LEN_PART) > self.max_frame_size {
                return Err(SomeipWireError::ExceedingMaxFrameSize)
            }
            if header.proto_version != ProtocolVersion::Version1 {
                return Err(SomeipWireError::UnsupportedProtocolVersion)
            }
        ...
        if self.wait_for_tp {
            if buf.len() < SOMEIP_TP_HEADER_SIZE {
                return Ok(None)
            }
...
mod tests {
    ...
    fn decode_someip_header_tp1() {
        ...
        assert_eq!(msg.0.tp_header, Some(SomeipTpHeader{offset: 0x14 , more: true}));
